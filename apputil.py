...
def sort_names(names, scores):
    ...
    Returns:
        list of tuples: Each tuple contains (name, score), sorted by score descending.
    """
    sorted_idx = np.argsort(scores)[::-1]  # Indices sorted by score (high → low)
    
    # Pair up each student with their score
    sorted_list = [(names[i], scores[i]) for i in sorted_idx]
...
# Expected: [('Hannah', 99), ('Jung', 91), ('Abdul', 85), ('Astrid', 71), ('Mauve', 62)]
